...
FileBuild.prototype.pushIf = function(value, array) {
  if (!this.inArray(value, array) && array) {
...
FileBuild.prototype.loadJsDepend = function(fileList, depsList) {
  var i, jsFilePath, len, results, __jsFilePath, jsFilePath_index;
  results = [];
    ...
    if (jsFilePath.indexOf('/') >= 0) {
      if (!path.isAbsolute(jsFilePath)) {
        ...
        if(!fs.existsSync(jsFilePath)){
          __jsFilePath = jsFilePath + "/index";
          jsFilePath_index = path.join(process.cwd(), "./node_modules/", jsFilePath + "/index.js");
          jsFilePath = jsFilePath_index;
          if(!fs.existsSync(jsFilePath_index)){
            __jsFilePath = jsFilePath + "/lib/index";
            jsFilePath = path.join(process.cwd(), "./node_modules/", jsFilePath + "/lib/index.js");
          }
        }
        ...
        if(!fs.existsSync(jsFilePath)){
          jsFilePath = jsFilePath.replace('.js', "/index.js");
        }
...
FileBuild.prototype.writeJs = function(filePath, depsList) {
  ...
  var _compressCode = _code;
  if (this.options.compress) {
    var ars = _compressCode.match(/require\(('|")(.*?(|.js))('|")\)/g);
    if(ars && ars.length > 0){
      for(var i=0, len = ars.length; i < len; i ++){
        var item = ars[i];
        if(item.indexOf('.js') > -1){
          _compressCode = _compressCode.replace(item, item.replace('.js', '-min.js'));
        }else{
          _compressCode = _compressCode.replace(item, item.replace(/^require\(('|")(.*?)('|")\)$/, "require('$2-min')"))
        }
      }
    }
